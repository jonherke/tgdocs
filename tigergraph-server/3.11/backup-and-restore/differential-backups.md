...
Page 1[Prev](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups)[Next](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups)
Search by [Algolia](https://www.algolia.com/docsearch)
[Products](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups)
TigerGraph Savanna
...
[Resources](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups)
...
[Edit this Page](https://github.com/tigergraph/server-docs/edit/3.11/modules/backup-and-restore/pages/differential-backups.adoc)
### Contents
  * [Terminology Definition](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_terminology_definition)
  * [How Differential Backup works](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_how_differential_backup_works)
  * [Enable Differential Backup](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_enable_differential_backup)
  * [Disable Differential Backup](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_disable_differential_backup)
  * [Limitations](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_limitations)
...
## [](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_terminology_definition)Terminology Definition
...
Additionally, for example, if you have _full backup A_ and _full backup B_ , _full backup A_ happened before _full backup B_. If you restore _full backup A_ , then the next differential backup is based on _full backup A_ , instead of _full backup B_ , although _full backup B_ was completed more recently.
## [](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_how_differential_backup_works)How Differential Backup works
To illustrate how differential backup works, take into account the user scenario below.
Example 1. User Scenario
  1. A user creates a full backup every _Sunday_ (Or Day `N`).
  2. During the week, between the two full backup days (_Sunday_ and _Next Sunday_) TigerGraph provides differential backups for every change since that first full backup.
  3. A differential backup is taken until the backup data is as large as the backup on _Sunday_
  4. Another full backup is taken on _Next Sunday_ and the circle repeats steps 2-4.


### [](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_enable_differential_backup)Enable Differential Backup
...
For more details on `gadmin backup create` see [Back up a Database Cluster Procedure](https://docs.tigergraph.com/tigergraph-server/4.2/backup-and-restore/backup-cluster#_procedure).  
---  
...
![backup list](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/_images/backup_list.png)
...
### [](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_disable_differential_backup)Disable Differential Backup
...
## [](https://docs.tigergraph.com/tigergraph-server/3.11/backup-and-restore/differential-backups#_limitations)Limitations
Differential backup does have some know limitations:
  ...
  * Differential Backup must based on last full snapshot backup and cannot be based on a previous days differential backup.
    * For example: If a user did a full backup on _Sunday_ and then one differential backup on _Monday_. Then, if the user wants to run a differential backup on _Tuesday_ , they will have to base it on _Sunday’s_ full backup and save all changes since then. So part of your _Tuesday_ differential backup is actually the same as _Monday’s_.
