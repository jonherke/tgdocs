...
Page 1[Prev](https://docs.tigergraph.com/tigergraph-server/3.11/installation/change-ip-or-hostname)[Next](https://docs.tigergraph.com/tigergraph-server/3.11/installation/change-ip-or-hostname)
Search by [Algolia](https://www.algolia.com/docsearch)
[Products](https://docs.tigergraph.com/tigergraph-server/3.11/installation/change-ip-or-hostname)
TigerGraph Savanna
...
[Resources](https://docs.tigergraph.com/tigergraph-server/3.11/installation/change-ip-or-hostname)
...
[](https://docs.tigergraph.com/home/)
  ...
  * [Change IP or Hostname](https://docs.tigergraph.com/tigergraph-server/3.11/installation/change-ip-or-hostname)


[Edit this Page](https://github.com/tigergraph/server-docs/edit/3.11/modules/installation/pages/change-ip-or-hostname.adoc)
...
If you need to change the IP address or hostname of a node or a cluster after TigerGraph is up and running, you can change them by running the following commands as the tigergraph admin user.
  ...
  2. Shut down services, as shown below:
...
The command `gadmin init etcd` initializes the etcd component of TigerGraph, while `gadmin stop all` stops all running TigerGraph services.
  1. Make your IP changes (outside of TigerGraph).
  2. Apply the new IP/hostnames, using the two commands below.


The first command applies the updated TigerGraph host list from '~/.tg.cfg' to define cluster nodes for deployment.
The second command initializes the TigerGraph cluster without stopping running services `(--skip-stop)` and also initializes etcd, the distributed key-value store for configuration and coordination `(--init-etcd)`.
```
